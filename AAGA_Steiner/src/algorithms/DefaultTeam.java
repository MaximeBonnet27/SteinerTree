...
import genetic.AlgorithmeGenetique;

public class DefaultTeam {
  public Tree2D calculSteiner(ArrayList<Point> points) {
    Tree2D res = tryAllFermats(points);
  ...
  public void barycentresSubAndSubSub(Tree2D tree) {
  ...
  // Configuration étudiée ici :
  // Fils - Racine - Fils
  public void barycentresSub(Tree2D tree) {
    ArrayList<Tree2D> trees = new ArrayList<Tree2D>();
    trees.add(tree);
    while (!trees.isEmpty()) {
      Tree2D current = trees.remove(0);
  ...
  public void barycentreMultiSubs(Tree2D tree) {
    ...
    for (xPos = xMin; xPos + width < xMax; xPos += width) {
      for (yPos = yMin; yPos + height < yMax; yPos += height) {
        ArrayList<Point> localPoints = pointsInWindow(listePoints, xPos, yPos,
            width, height);
        boolean found = false;
        for (int i = 0; i < localPoints.size() - 2; ++i) {
          Point p = localPoints.get(i);
          for (int j = i + 1; j < localPoints.size() - 1; ++j) {
            ...
            for (int k = j + 1; k < localPoints.size(); ++k) {
              ...
              if (betterWithBarycentre(p, q, r)) {
                ...
                if(pTree.getSubTrees().size() >= 2) continue;
                if(qTree.getSubTrees().size() >= 2) continue;
                if(rTree.getSubTrees().size() >= 2) continue;
  ...
  public ArrayList<Point> bestFermatPoints(ArrayList<Point> points){
    HashMap<Point, Integer> map = new HashMap<>();
    for(int i = 0; i < 100; ++i){
      System.out.println("i = " + i);
      Tree2D tree = Prim.compute(i,points);
      int bestScore = tree.score();
      int score = Integer.MAX_VALUE;
      for(int iterations = 0; iterations < 100; ++iterations){
        barycentreMultiSubs(tree);
        barycentresSub(tree);
        barycentresSubAndSubSub(tree);
      }
      for(Point p : tree.getListePoints()){
    ...
    for(Point p : map.keySet()){
      if(map.get(p) >= 95 || map.get(p) <= 2){
        res.add(p);
      }
    }
    System.out.println("Ajout de " + res.size());
    return res;
  }

  public Tree2D tryAllFermats(ArrayList<Point> points){
    ArrayList<Point> fermats = bestFermatPoints(points);
    int bestScore = Integer.MAX_VALUE;
    int score;
    Tree2D res = null;
    for(long mask = (1L << (fermats.size())) - 1; mask > 0; --mask){
      ArrayList<Point> clones = (ArrayList<Point>) points.clone();
      for(int i = 0; i < fermats.size(); ++i){
        if((mask & (1L << i)) != 0){
          clones.add(fermats.get(i));
        }
      }
      int iterations = 0;
      Tree2D tmp = Prim.compute(clones);
      while(iterations++ < 10){
